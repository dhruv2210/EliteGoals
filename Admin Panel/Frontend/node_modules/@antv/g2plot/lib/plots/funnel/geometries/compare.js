...
function compareConversionTag(params) {
    ...
    (util_1.isNumber(index) ? [chart] : chart.views).forEach(function (view, viewIndex) {
        // 获取形状位置，再转化为需要的转化率位置
        var dataArray = util_1.get(view, ['geometries', '0', 'dataArray'], []);
        var size = util_1.get(view, ['options', 'data', 'length']);
        var x = util_1.map(dataArray, function (item) { return util_1.get(item, ['0', 'nextPoints', '0', 'x']) * size - 0.5; });
