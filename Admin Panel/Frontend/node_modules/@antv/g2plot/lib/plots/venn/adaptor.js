...
function colorMap(params, data, colorPalette) {
    ...
    var map = utils_2.getColorMap(palette, data, blendMode, setsField);
...
function data(params) {
    ...
    var filterSets = data.filter(function (datum) {
        var sets = datum.sets;
        // 存在非法元素，就过滤这条数据
        return utils_2.islegalSets(currSets, sets);
    });
    if (!util_1.isEqual(filterSets, data))
        utils_1.log(utils_1.LEVEL.WARN, false, 'warn: %s', '交集中不能出现不存在的集合, 请输入合法数据');
...
function geometry(params) {
    ...
    var vennData = utils_2.layoutVennData(options, Math.max(width - (r + l), 0), Math.max(height - (t + b), 0), 0);
    chart.data(vennData);
