...
function adapteStyle(style) {
    ...
    var _a = pick_1.pick(style, shapeStyleKeys), shadowColor = _a.shadowColor, _b = _a.shadowBlur, shadowBlur = _b === void 0 ? 0 : _b, _c = _a.shadowOffsetX, shadowOffsetX = _c === void 0 ? 0 : _c, _d = _a.shadowOffsetY, shadowOffsetY = _d === void 0 ? 0 : _d;
    styleObject['text-shadow'] = "" + [shadowColor, shadowOffsetX + "px", shadowOffsetY + "px", shadowBlur + "px"].join(' ');
    var _e = pick_1.pick(style, shapeStyleKeys), stroke = _e.stroke, _f = _e.lineWidth, lineWidth = _f === void 0 ? 0 : _f;
    ...
    util_1.each(style, function (v, k) {
        ...
        if (['fontSize'].includes(k) && util_1.isNumber(v)) {
            styleObject[kebab_case_1.kebabCase(k)] = v + "px";
        }
        else if (k && !shapeStyleKeys.includes(k)) {
            styleObject[kebab_case_1.kebabCase(k)] = "" + v;
        }
    });
    return styleObject;
}
...
exports.renderStatistic = function (chart, options, datum) {
    ...
    [titleOpt, contentOpt].forEach(function (option, idx) {
        ...
        chart.annotation().html(tslib_1.__assign({ position: ['50%', '50%'], html: function (container, view) {
            ...
            // @ts-ignore
            key: (idx === 0 ? 'top' : 'bottom') + "-statistic" }, pick_1.pick(option, ['offsetX', 'offsetY', 'rotate', 'style', 'formatter']) /** 透传配置 */));
    });
};
/**
...
exports.renderGaugeStatistic = function (chart, options, datum) {
    ...
    [titleOpt, contentOpt].forEach(function (option) {
        ...
        chart.annotation().html(tslib_1.__assign({ position: ['50%', '100%'], html: function (container, view) {
                ...
                // todo G2 层修复可以返回空字符串 & G2 层修复允许返回非字符串的内容，比如数值 number
                return text ? (util_1.isString(text) ? text : "" + text) : '<div></div>';
            } }, pick_1.pick(option, ['offsetX', 'offsetY', 'rotate', 'style', 'formatter']) /** 透传配置 */));
    });
};
